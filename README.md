# 100 Days WebDeveloper Journey 🚀

I am documenting my progress in learning programming. My goal is to master Full-Stack Web Development at a junior level in 100 days.

## 📚 What I'm Learning

**Technologies:**
- HTML5 & CSS3
- JavaScript (ES6+)
- React
- Node.js & Express
- PostgreSQL
- Web3 & DApps
- Git & GitHub

## 🎓 Main Course

**[The Complete Full-Stack Web Development Bootcamp](https://www.udemy.com/course/the-complete-web-development-bootcamp/)** by Dr. Angela Yu
- 61 hours of video content
- 16 web development projects for portfolio
- HTML, CSS, JavaScript, Node, React, PostgreSQL, Web3

## 📅 Progress Tracker

**Week 1-2:** HTML & CSS Fundamentals 🔄  
**Week 3-4:** JavaScript Basics  
**Week 5-6:** Advanced JavaScript  
**Week 7-8:** React  
**Week 9-10:** Backend with Node.js  
**Week 11-12:** Databases & Final Projects  

*(Last updated: January 28, 2026)*

## 🎯 Goals

- ✅ Complete the Udemy Full-Stack Bootcamp
- [ ] Build 5 portfolio-ready projects
- [ ] Contribute to 3 open source projects
- [ ] Land my first junior developer job

## 📂 Repository Structure

```
/html-css/
  /flexbox-practice
  /grid-layouts
  /responsive-design
/javascript/
  /basics
  /dom-manipulation
  /async-programming
/react/
  /components
  /hooks
  /projects
/backend/
  /node-express
  /databases
/projects/
  /portfolio-website
  /todo-app
  /weather-app
/notes/
  /javascript-cheatsheet.md
  /react-notes.md
  /git-commands.md
```

## 🔗 Learning Resources

### Primary Course
- [The Complete Full-Stack Web Development Bootcamp (Udemy)](https://www.udemy.com/course/the-complete-web-development-bootcamp/)

### Supplementary Resources
- [FreeCodeCamp](https://freecodecamp.org)
- [MDN Web Docs](https://developer.mozilla.org)
- [JavaScript.info](https://javascript.info)
- [The Odin Project](https://www.theodinproject.com)

## 💡 Daily Commitment

- Minimum 2 hours of coding daily
- Regular commits to track progress
- Document learnings and challenges
- Build real projects, not just tutorials

## 🛠️ Projects Built

*Will be updated as I complete projects*

1. **Personal Portfolio** - HTML, CSS, JavaScript
2. **To-Do App** - React, Local Storage
3. **Weather App** - React, API Integration
4. **Blog Platform** - Full-Stack (Coming soon)
5. **E-commerce Site** - Full-Stack (Coming soon)

## 📈 Stats

![GitHub streak](https://github-readme-streak-stats.herokuapp.com/?user=Bergyx&theme=dark)

## 📝 What I Learned Today

Check my recent commits to see what I'm currently working on!

---

**Start Date:** January 26, 2026  
**Target Completion:** May 2026  

*"The expert in anything was once a beginner."*

---

### Connect with me:
- GitHub: [@Bergyx](https://github.com/Bergyx)
- Twitter: [@SzymonBerg](https://twitter.com/SzymonBerg)

---

⭐ Star this repo if you're also on a coding journey!
